...
- 当前结果：
  ...
  - `turnAccess / 失先手代价 / 残局安全起手值` 仍是这一里程碑的主要未完成部分。
- 验收：
...
1. 继续补 `里程碑 1` 中与残局续控相关的扩展触发，让中级 AI 真正看到“这一墩之后会怎样”。
2. 然后推进 `里程碑 3`，把“失先手代价”“残局安全起手”“turnAccess”真正沉到统一评估里。
3. 同步扩展 `里程碑 3.5`，把甩牌风险和牌权流向都补成专项回归，而不只停留在基础回归里。
4. 最后做 `里程碑 4` 的性能和调试保护，给后续继续调权重留足空间。
