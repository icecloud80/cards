...
- 仓库仍保留 `m_cards_sprite.svg` 的生成链路，方便后续资源实验或对照；但当前运行态默认整图牌面已经整体恢复为 `poker.png`。这只改渲染资源入口，不改变 AI 的规则边界、难度差异和评分逻辑。
- 旧的 `modern-sprite` 存档键值仍会兼容映射到当前 `sprite` 主题；这仍然只是表现层兼容处理，不改变 AI 输入、评分或 heuristic。
