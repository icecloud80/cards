...
- `高价值朋友牌非赢墩不亮` 这条共享跟牌收口也已补上：
  当当前亮友候选本身是 `大王 / 高主 / A/K`，而这手打出去又拿不到当前轮次时，
  beginner / intermediate / advanced 会先找不含朋友牌的合法低成本替代跟牌，不再为了“顺手站队”白白交掉这张后续控牌。
...
- 2026-03-19 当前工作区又补了一条“叫死第三张 A 的无主接手”收口：
  当打家在无主局里已经把朋友牌叫死为唯一外部副本，例如自己整理成 `AA10`、目标是第三张 `A` 时，
...
- PC、mobile 与 App 顶部新增的 `刷新` 图标同样只属于局内流程控制：它只会保留当前级别并按当前 `replaySeed` 重新发牌，不会改变 AI 难度、策略边界或信息利用能力。
- mobile 设置菜单现已移除重复的 `刷新` 按钮；这次只是把同一能力收口到顶部高频入口，不涉及任何 AI 策略、评分或 heuristic 调整。
- App 默认入口切到 `index-app.html`、并把牌桌改成“顶部固定 / 中部自适应 / 底部固定”，同样只属于原生壳布局修复；它不会改变 AI 难度定位、评分逻辑或任何策略边界。
- `index1 / index2 / index-app` 的底部主操作区现已统一收口为“只留放大的 `选择 / 出牌`，其余抓牌 / 反主 / 扣牌等待状态全部走共享 `setupOptions` 面板”；这同样只属于人类交互入口整理，不改变 AI 的亮主 / 反主策略、评分或信息边界。
...
- PC、mobile 与 App 顶栏现已统一改成 `刷新` 图标入口，并明确限定为“保级复用当前 replay seed 重开本局”；这次变更只影响共享状态机与 UI 入口，不改变任何 AI 评估和 heuristic。
...
- 本次改动只修正人类玩家的声明 UI：三端共享 setup 面板统一承接 `不亮 / 不反 / 玩家扣牌中` 等状态与跳过动作。
