...
- 在 iOS 与 Android 主屏上保持同一套红金品牌气质，同时兼容 Android adaptive icon 的裁切限制。
...
- 背景使用深红到酒红的纵向渐变，并叠加轻微菱形纹理，呼应牌背与徽章切面。
...
- 约束：
  ...
  - adaptive background 必须继续保持红金品牌渐变，避免回退成默认白底。
...
- 若未来需要补商店宣传图、开屏或首页品牌插画，应优先沿用这套 `红金 + 叠牌 + 升级徽章` 的主品牌语言，避免 App 首页和桌面图标分裂成两套风格。
