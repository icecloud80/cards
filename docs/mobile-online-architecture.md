...
- 建议拆成：
  - `Preferences`：设置、牌面、节奏、跨局托管态、玩家等级进度、最近一局复盘输入。
  ...
  - 这轮 `round` 只保留 `回放种子 + 开局码` 两项，用于日志、QA 和手动重建开局；不保存 phase、手牌或中途牌局快照。
  - `SQLite` 或本地数据库`/`完整牌局快照恢复留到下一阶段，不在这轮轻量迁移里落地。

### 4.2 生命周期

- 这轮先不承诺“恢复到中断时刻继续打”。
- App 重启后只要求最近一局 `回放种子 + 开局码` 仍可读，用于从开局重建和问题复盘。
- 真正的中途牌局快照、随机状态续接与未完成对局恢复留到后续阶段。
