...
- 2026-03-19 - 【AI改进】 - 初级 AI 的跟牌又补了一条更窄的共享 heuristic：当别人出对子，而自己这门正好是 `刻子 + 两张杂牌` 时，现在会优先用这两张同门杂牌合法跟牌，不再因为“拆成对子还能压住当前”就机械拆刻子。对应 `AI friend strategy` 已新增 `pair-follow preserves exact triple` 回归，并同步更新规则摘要、beginner heuristic、AI 路线图与状态文档；本轮不改 PC / mobile UI。
