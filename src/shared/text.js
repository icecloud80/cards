const TEXT = {
  ...
  scorePanel: {
    ...
    callingFriendSelf: "你已扣底完成，请在 30 秒内叫朋友；确认后读秒结束前，还能点顶部朋友牌再改一次。",
    callingFriendOther: "打家正在叫朋友，稍后进入正式出牌。",
    ending: "本局已出完最后一张牌，正在整理结算结果。",
    unresolvedFriend: "朋友未站队前，抓分先记在各玩家自己名下。",
    pause: "本轮暂停中，准备进入下一轮",
  ...
  hand: {
    ...
    dealingCanCounter: (count, options) => `当前共 ${count} 张，已经有人亮主；你可以用 ${options.join(" / ")} 继续反主。`,
    dealingNoCounter: (count) => `当前共 ${count} 张，已经有人亮主；你暂时没有更高一档的合法反主组合。`,
    ...
    callingFriendSelf: (count) => `当前共 ${count} 张，请先在弹出的菜单里叫朋友。推荐按钮会显示 30 秒读秒，确认后读秒内还能点顶部朋友牌再改一次。`,
  ...
  actionHint: {
    ...
    dealingCanCounter: (options) => `发牌中。当前已有亮主，你可以立即用 ${options.join(" / ")} 反主；点“不反”表示当前这手先不反。`,
    dealingCounterWait: "发牌中。当前已有亮主，你暂时没有更高一档的合法反主组合；可以先点“不反”。",
    ...
    callingFriendSelf: "请先叫朋友。推荐按钮会显示 30 秒读秒；确认后在读秒内，还能点顶部朋友牌再改一次。",
  ...
  debug: {
    title: "调试看牌",
    replaySection: "复原开局",
    replayHint: "想完整回到原局，请同时填写开局码和回放种子。只填回放种子只会重建随机链路，不保证恢复同一手牌。",
    replaySeedLabel: "回放种子",
    replaySeedPlaceholder: "例如：02R9g6M4wQK",
    replaySeedApply: "仅按回放种子重开",
    openingCodeLabel: "开局码",
    openingCodePlaceholder: "粘贴日志里的 169 位字母数字混合开局码（区分大小写）",
    ...
    replayInvalidOpeningCode: "开局码无效，请确认已完整粘贴日志中的字母数字混合文本，并保留原始大小写。",
  ...
  rules: {
    ...
    validation: {
      ...
      tripleFollowPair: "首家出刻子时，没有刻子也要尽量跟一对；有两对也只需要跟一对，不用拆刻子。",
      trainMustFollow: "首家出拖拉机、火车或宇宙飞船时，你有可跟的拖拉机 / 火车 / 宇宙飞船结构就必须先跟连对。",
      trainFollowPairs: "首家出拖拉机、火车或宇宙飞船时，连对不够也要尽量跟对子；有几对跟几对，刻子不用拆对。",
      bulldozerMustFollow: "首家出推土机时，你有同长度推土机就必须跟推土机。",
      bulldozerTriples: "首家出推土机时，你有刻子就必须先跟刻子；有两副刻子时要跟两副刻子。",
      bulldozerPairs: "首家出推土机时，没有足够刻子也要尽量跟对子；最多跟两对，不需要拆刻子去凑三对。",
  ...
  log: {
    ...
    passDeclare: (name) => `${name} 选择不亮，直接进入翻底定主。`,
    ...
    friendRevealed: (name, target) => `${name} 打出了${target}，已站队。`,
    teamsRevealed: (points) => `阵营已明，闲家当前累计 ${points} 分。`,
    ...
    finalBottomPenalty: (label, levels) => `闲家以${label}完成级牌扣底，直接获胜，打家降 ${levels} 级。`,
 ...
 *
 * 为什么这样写：
 * 现在扣底已经拆成“普通扣底 / 级牌扣底”两条规则：
 * 普通扣底继续只看总分是否到 120，级牌扣底则会直接判闲家获胜。
 * 把整局结算统一收口在这里，能避免规则层、结果页和日志各自维护一套分支。
 *
 * 输入：
 * @param {number} points - 已经包含底牌加分后的闲家最终总分
 * @param {{bottomPenalty?: {levels: number, label: string, mode?: string, kind?: string} | null}} options - 级牌扣底惩罚信息
 *
 * 输出：
 ...
 * - 普通扣底下，闲家仍需达到 120 分才算赢
 * - 级牌扣底下，闲家即使不到 120 分也会直接获胜
 */
function getOutcome(points, options = {}) {
  const gradeBottomWin = options.bottomPenalty?.levels > 0;
  const defendersWin = gradeBottomWin || points >= 120;
  if (gradeBottomWin && points < 165) {
    return {
      title: TEXT.outcome.defenderWinTitle,
      body: points < 120
        ? `闲家总分为 ${points} 分，虽未到 120 分，但已完成${options.bottomPenalty.label}；按级牌扣底直接获胜，本局不升级。`
  ...
  if (!defendersWin && points === 0) {
    return {
      ...
      body: "闲家总分为 0 分，且未形成级牌扣底，打家方升 3 级。",
      bankerLevels: 3,
      defenderLevels: 0,
      winner: "banker",
    };
  ...
  if (!defendersWin && points < 60) {
    return {
      ...
      body: `闲家总分为 ${points} 分，未到 60 分，且未形成级牌扣底，打家方升 2 级。`,
      bankerLevels: 2,
      defenderLevels: 0,
      winner: "banker",
    };
  ...
  if (!defendersWin && points < 120) {
    return {
      ...
      body: `闲家总分为 ${points} 分，未到 120 分，且未形成级牌扣底，打家方正常获胜，升 1 级。`,
      bankerLevels: 1,
      defenderLevels: 0,
      winner: "banker",
    };
  ...
  return {
    ...
    body: gradeBottomWin
      ? `闲家总分为 ${points} 分，并以${options.bottomPenalty.label}完成级牌扣底。本局闲家方升 ${levels} 级。`
      : `闲家总分为 ${points} 分，按你当前规则从 165 分开始升级，本局闲家方升 ${levels} 级。`,
    bankerLevels: 0,
...
function getBottomResultText(bottomResult) {
  ...
  if (bottomResult.penalty) {
    return ` ${bottomResult.playerName}完成扣底，并以${bottomResult.penalty.label}触发级牌扣底；本局直接判闲家获胜，打家降 ${bottomResult.penalty.levels} 级。下一局由玩家${bottomResult.nextLeadPlayerId}先抓牌。`;
  }
  return ` ${bottomResult.playerName}完成普通扣底；本次只计底牌分，是否获胜仍看闲家最终总分。下一局由玩家${bottomResult.nextLeadPlayerId}先抓牌。`;
}
