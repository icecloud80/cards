const TEXT = {
  ...
  friend: {
    ...
    pickerPreview: (label) => `当前要找：${label}`,
  ...
  scorePanel: {
    ...
    callingFriendSelf: "你已扣底完成，请先找朋友，再开始出牌。",
    callingFriendOther: "打家正在找朋友，稍后进入正式出牌。",
  ...
  hand: {
    ...
    callingFriendSelf: (count) => `当前共 ${count} 张，请先在弹出的菜单里找朋友，再进入首轮出牌。`,
    callingFriendOther: (count) => `当前共 ${count} 张，等待打家找朋友。`,
  ...
  actionHint: {
    ...
    callingFriendSelf: "请先找朋友。通常会先选一门花色，再选点数，确认后才进入正式出牌。",
    callingFriendOther: "打家正在找朋友，请稍候。",
  ...
  log: {
    ...
    startCallingFriend: (name) => `${name} 已扣底完成，当前需要先找朋友，再进入出牌。`,
    friendCalled: (label) => `已找朋友：${label}。`,
    ...
    teamsRevealed: (points) => `阵营已明确，闲家当前累计 ${points} 分。`,
