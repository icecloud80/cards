 ...
 *
 * 注意：
 * - 这里既覆盖主扣也覆盖副扣，确保 `A` 进入负级链后不再受旧锚点分叉影响。
...
function runSuite() {
  ...
  assert.equal(context.dropLevel("A", 1, "trump"), "-K", "A 主扣 1 级后应先落到 -K");
  assert.equal(context.dropLevel("A", 2, "vice"), "-Q", "A 副扣 2 级后应继续按 -K -> -Q 顺序回退");
  assert.equal(context.dropLevel("A", 20, "trump"), "-2", "A 连续降级后应以 -2 为最低档");
  assert.equal(context.dropLevel("-K", 1, "vice"), "-Q", "负级内部继续降级时应沿完整负级链顺序回退");
  assert.equal(context.shiftLevel("-2", 1), "-3", "负级升级时应在负级链内逐档回升");
  assert.equal(context.shiftLevel("-Q", 3), "2", "负级回到正级时单次结算最多只能回到 2");
...
console.log(`- A first drop target: ${output.aceDropTarget}`);
