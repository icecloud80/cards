...
function main() {
  const expectedItems = [
    ...
    "首家只能出同一门的合法牌型；跟牌先跟同门，同门不够就全出。缺首门后可以贴副或用主毙，贴副时不用硬贴对子或连组。",
    ...
    "打 J / Q 时要特别留意：这两个特殊级里，主级牌扣底和副级牌扣底都会算，不是只有主级牌才危险。",
    "A 级之后再被级牌扣底，会进入负级链；局内统一简写成 Lv:-K ... Lv:-2。主 K 扣回 J，副 A 扣回 K。",
